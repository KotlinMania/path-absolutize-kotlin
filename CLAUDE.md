# Claude Code Project Instructions — path-absolutize-kotlin

## Project Overview

This is **path-absolutize-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`path-absolutize`](https://crates.io/crates/path-absolutize).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/path-absolutize/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/path-absolutize>
