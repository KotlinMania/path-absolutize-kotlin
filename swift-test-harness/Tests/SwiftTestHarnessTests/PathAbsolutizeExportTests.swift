...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `PathAbsolutize.swiftmodule/`
...
//      `import PathAbsolutize` above would fail at compile time.
//
//   2. The static archive `libPathAbsolutize.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "PathAbsolutize" }` and
//      `flattenPackage = "io.github.kotlinmania.pathabsolutize"` configuration in
...
//      via the `PathAbsolutizeLibrary` product.
//
...
final class PathAbsolutizeExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "PathAbsolutize swift module imported cleanly")
    }
}
